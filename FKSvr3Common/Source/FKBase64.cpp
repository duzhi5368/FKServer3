...
//------------------------------------------------------------------------
const BYTE def_base64_decode_map[256] = { 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
...
stBase64Setup::stBase64Setup(){
	strcpy_s(base64_map,sizeof(base64_map),def_base64_map);
...
void stBase64Setup::init_base64_map(char* pmap,char mod){
	if (strcmp(base64_map,pmap)!=0 && strchr(pmap,mod)==0){
		base64_mod=mod;
		strcpy_s(base64_map,sizeof(base64_map),pmap);
		memset(base64_decode_map,-1,sizeof(base64_decode_map));
		for (int i=0;i<sizeof(base64_map);i++){
			if (base64_map[i]!=0){
				base64_decode_map[base64_map[i]]=(BYTE)i;
			}
		}
	}
}
	...
	if (dst_len>=decode_len){
		...
		for (; i < src_len - nmod; i += 4) {
			dst[j++] = (base64_decode_map[src[i]] << 2) |
				((base64_decode_map[src[i + 1]] >> 4) & 0x03);
			dst[j++] = (base64_decode_map[src[i + 1]] << 4) |		
				((base64_decode_map[src[i + 2]] >> 2) & 0x0f);
